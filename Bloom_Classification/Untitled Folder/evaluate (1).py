...
def train_val_iou(train_iou, val_iou, epochs, save = True, fig_name=''):
    epoch = range(epochs)
    fig, ax = plt.subplots(1,1, figsize = (10,6))   
    ax.plot(epoch, train_iou, color='b', linewidth=0.5, label='Training')
    ax.plot(epoch, val_iou, color='r', linewidth=0.5, label='Validation')
...
def train_val_dice(train_dice,val_dice, epochs, save = True, fig_name=''):
    epoch = range(epochs)
    fig, ax = plt.subplots(1,1, figsize = (10,6))   
    ax.plot(epoch, train_dice, color='b', linewidth=0.5, label='Training')
    ax.plot(epoch, val_dice, color='r', linewidth=0.5, label='Validation')
...
def plot_batch(x, y, preds):
    """
    Plot all images, ground truth, and predictions in a batch.

    Parameters:
        x (torch.Tensor): Input images of shape [B, 3, 224, 224].
        y (torch.Tensor): Ground truth labels of shape [B, 5, 5].
        preds (torch.Tensor): Predicted labels of shape [B, 5, 5].
    """
    # Colors and colormap setup
    colors = ['black', 'seagreen', 'yellow', 'grey']  # Class colors
    cmap = ListedColormap(colors)
    boundaries = [0, 1, 2, 3]  # Each boundary represents a class (0, 1, 2, 3)
    norm = BoundaryNorm(boundaries, cmap.N)
    ...
    for i in range(batch_size):
        x_img = x[i].permute(1, 2, 0).cpu().numpy()  # Normalize input image
        x_img = (x_img - x_img.min()) / (x_img.max() - x_img.min())

        y_label = y[i].cpu().numpy()  # Ground truth
        preds_label = preds[i].cpu().numpy()  # Predictions
        ...
        # Ground truth
        im2 = axes[i, 1].imshow(y_label, cmap=cmap, norm=norm)
        axes[i, 1].set_title(f"Ground Truth {i}")
        axes[i, 1].axis('off')

        # Predictions
        im3 = axes[i, 2].imshow(preds_label, cmap=cmap, norm=norm)
        axes[i, 2].set_title(f"Predictions {i}")
        axes[i, 2].axis('off')
